(() => {
  ...
  function reset(){ snake=[{x:10,y:10}]; dir={x:1,y:0}; nextDir={x:1,y:0}; score=0; scoreEl.textContent='0'; spawnFood(); clearInterval(timer); timer=setInterval(tick, 120); draw(); }
  ...
  function tick(){ dir = nextDir; const head = {x: snake[0].x + dir.x, y: snake[0].y + dir.y};
    ...
    snake.unshift(head);
    if(head.x===food.x && head.y===food.y){ score += 10; scoreEl.textContent=String(score); spawnFood(); }
    else snake.pop();
    draw();
  }
  function draw(){ ctx.clearRect(0,0,canvas.width,canvas.height); ctx.strokeRect(0.5,0.5,canvas.width-1,canvas.height-1); snake.forEach((s,i)=>{ ctx.fillRect(s.x*size+2,s.y*size+2,size-4,size-4); }); ctx.beginPath(); ctx.arc(food.x*size + size/2, food.y*size + size/2, size/2.8, 0, Math.PI*2); ctx.fill(); }
