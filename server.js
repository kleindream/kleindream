...
function normalizeAvatarKleinConfig(raw) {
  let data = raw || {};
  if (typeof data === "string") {
    try { data = JSON.parse(data); } catch (_) { data = {}; }
  }
  const cfg = { ...AVATAR_KLEIN_DEFAULT };
  for (const [key, max] of Object.entries(AVATAR_KLEIN_OPTIONS)) {
    const value = Number(data[key]);
    const min = (key === "oculos" || key === "barba") ? 0 : 1;
    cfg[key] = Number.isInteger(value) && value >= min && value <= max ? value : cfg[key];
  }
  return cfg;
}
...
app.get("/meu-avatar", requireAuth, async (req, res, next) => {
  try {
    const me = await db.get("SELECT id, username, avatar_config FROM users WHERE id=?", [req.session.userId]);
    if (!me) return res.redirect("/login");
    res.render("meu_avatar", {
      user: me,
      avatarConfig: normalizeAvatarKleinConfig(me.avatar_config),
      avatarOptions: AVATAR_KLEIN_OPTIONS
    });
  } catch (err) {
    next(err);
  }
...
app.post("/meu-avatar", requireAuth, limiterWrite, async (req, res, next) => {
  try {
    const cfg = normalizeAvatarKleinConfig(req.body || {});
